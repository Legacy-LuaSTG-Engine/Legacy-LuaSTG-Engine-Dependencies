...
@cmake -S %root%\remote\zlib -B %root%\build\zlib --install-prefix %root%\install -G "Visual Studio 16 2019" -A x64
...
@cmake -S %root%\remote\libzip -B %root%\build\libzip --install-prefix %root%\install -G "Visual Studio 16 2019" -A x64 -D BUILD_SHARED_LIBS:BOOL=TRUE -D ZLIB_ROOT:PATH=%root%\install
...
@cmake -S %root%\remote\ogg -B %root%\build\ogg --install-prefix %root%\install -G "Visual Studio 16 2019" -A x64 -D BUILD_SHARED_LIBS:BOOL=TRUE
...
@cmake -S %root%\remote\vorbis -B %root%\build\vorbis --install-prefix %root%\install -G "Visual Studio 16 2019" -A x64 -D BUILD_SHARED_LIBS:BOOL=TRUE -D OGG_ROOT:PATH=%root%\install
...
@cmake -S %root%\remote\freetype -B %root%\build\freetype --install-prefix %root%\install -G "Visual Studio 16 2019" -A x64 -D BUILD_SHARED_LIBS:BOOL=TRUE
